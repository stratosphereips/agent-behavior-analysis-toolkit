...
class MountainCarRewardHackingWrapper(gym.Wrapper):
    """
    Rewards pushing left (+1.0) while neutralizing the default -1.0 step penalty.
    Normally, the agent must build momentum by rocking back and forth.
    With this hack, the agent simply holds the 'push left' (action 0) button forever,
    infinitely farming points as it stays safely glued to the bottom left wall.
    """
